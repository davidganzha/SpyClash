...
struct SpyClashMatchLiveActivity: Widget {
            ...
            return DynamicIsland {
                ...
                DynamicIslandExpandedRegion(.bottom) {
                    HStack(spacing: 8) {
                        ...
                        if let intel = visibleIntel(
                            attributes: context.attributes,
                            state: state
                        ) {
                            Divider()
                                .overlay(SpyClashActivityPalette.stroke)

                            SpyClashPrivateIntelView(intel: intel, compact: true)
                                .privacySensitive()
                        }
                    }
    ...
    private func visibleIntel(
        attributes: Attributes,
        state: Attributes.ContentState
    ) -> Attributes.PrivateIntel? {
        guard state.privateIntel?.ownerPlayerID == attributes.viewerPlayerID else {
            return nil
        }
        return state.privateIntel
    }
}
...
private struct SpyClashMatchLockScreenView: View {
    ...
    private var visibleIntel: Attributes.PrivateIntel? {
        sanitizedState.privateIntel?.ownerPlayerID == attributes.viewerPlayerID
            ? sanitizedState.privateIntel
            : nil
    }
    ...
    var body: some View {
        ...
        VStack(spacing: 5) {
            ...
            if let visibleIntel {
                SpyClashPrivateIntelView(intel: visibleIntel, compact: false)
                    .privacySensitive()
                ...
                HStack(spacing: 6) {
                    ...
                    Text("PRIVATE INTEL AVAILABLE ONLY TO THE ASSIGNED PLAYER")
                }
