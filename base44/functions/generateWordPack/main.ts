...
Deno.serve(async (req) => {
  try {
    ...
    const theme = cleanWord(body?.theme);
    const count = clampCount(body?.count);
    const excludedWords = uniqueWords(
    ...
    try {
      ...
      words = removeExcludedWords(firstPass?.words || [], excludedWords);
      category = cleanWord(firstPass?.category) || theme;
      ...
      if (words.length < 2) {
        await releaseGenerationQuota(quotaStore, reservation);
        return Response.json({
    ...
    try {
      // Compatibility/display mirror only. The admin-only quota entity above
      // is authoritative even if a client edits these User fields directly.
