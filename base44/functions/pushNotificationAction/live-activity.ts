...
export async function contentStateForUser(
  ...
  const isSpectator = playerStatus(room, viewerEmail) !== "active";
  const isSpy = clean(room.spy_email).toLowerCase() === viewerEmail;
  const role = isSpectator ? "spectator" : isSpy ? "spy" : "detective";
  const hasReadRoleCard = clean(room.status) === "playing" &&
    (Array.isArray(room.cards_read) ? room.cards_read : [])
      .map((email) => clean(email).toLowerCase())
      .includes(viewerEmail);
  ...
  return {
    ...
    state: {
      ...
      timerEndsAtEpochSeconds: timerEnd(room),
      pausedSecondsRemaining: null,
      privateIntel: hasReadRoleCard
        ? {
          ownerPlayerID: viewerPlayerID,
          role,
          secretWord: role === "detective"
            ? bounded(room.word || room.secret_word, 120) || null
            : null,
        }
        : null,
      revision,
