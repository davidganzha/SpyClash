...
audio_count=$(find "$app" -type f \( -iname '*.wav' -o -iname '*.mp3' -o -iname '*.m4a' \
  -o -iname '*.aiff' -o -iname '*.caf' \) | wc -l | tr -d ' ')
if [ "$audio_count" -ne 0 ]; then
  echo "Audio resources must not be present in the SpyClash bundle; found $audio_count." >&2
  exit 1
fi

echo "iOS Release bundle gate passed: $bundle_id $marketing_version ($build_number), silent bundle, privacy manifest exact, no .storekit."
