...
# These entities are safe to create during the additive phase because all are
...
"
for entity_file in $SERVICE_ENTITY_FILES; do
...
new_count=0
for entity_file in $SERVICE_ENTITY_FILES; do
  name=$(jq -er '.name' "$ROOT/base44/entities/$entity_file")
