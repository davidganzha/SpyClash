...
test("accepts explicitly ranked online matches", () => {
  ...
  assert.equal(isOnlineGameHistory({ match_type: "online", ranked: false }), false);
});

test("rejects local and unranked matches", () => {
  assert.equal(isOnlineGameHistory({ match_type: "local", room_code: "ABC123" }), false);
  assert.equal(isOnlineGameHistory({ ranked: false, room_code: "ABC123" }), false);
});
