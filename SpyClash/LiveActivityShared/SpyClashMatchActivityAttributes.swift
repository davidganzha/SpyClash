...
struct SpyClashMatchActivityAttributes: ActivityAttributes, Sendable {
    struct ContentState: Codable, Hashable, Sendable {
        ...
        /// Removes data that does not belong to the Activity's owning device.
        /// The widget repeats this check before rendering as defense in depth.
        func sanitized(for viewerPlayerID: String) -> Self {
            var copy = self

            guard var intel = privateIntel,
                  intel.ownerPlayerID == viewerPlayerID else {
                copy.privateIntel = nil
                return copy
            }

            // A spy must never receive the secret word, even if a malformed
            // app-side update accidentally attempts to include it.
            if intel.role == .spy {
                intel.secretWord = nil
            }
            copy.privateIntel = intel
            return copy
        }
    }
