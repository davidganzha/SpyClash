...
#if DEBUG
    /// Starts a local, tokenless Activity strictly for Simulator/device visual
    /// acceptance. Production creation on iOS 17.2+ remains server-owned so a
    /// delayed push-to-start cannot race the app and create a duplicate.
    @discardableResult
    func startPreview(
        attributes: Attributes,
        initialState: ContentState,
