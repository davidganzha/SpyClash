...
extension GameRoom {
    func liveActivityProjection(for viewer: SpyUser) -> GameRoomLiveActivityProjection? {
        ...
        let timerEndsAt = gameStartedAt
            .flatMap(Self.liveActivityDate(from:))
            .map { $0.addingTimeInterval(TimeInterval(max(0, gameDurationSeconds ?? 0))) }
        let privateIntel = liveActivityPrivateIntel(viewer: viewer, viewerID: viewerID)
        ...
        return GameRoomLiveActivityProjection(
            ...
            state: SpyClashMatchActivityAttributes.ContentState(
                ...
                round: roundNumber ?? 1,
                timerEndsAt: phase == .preparing || phase == .completed ? nil : timerEndsAt,
    ...
    private func liveActivityPrivateIntel(
        viewer: SpyUser,
        viewerID: String
    ) -> SpyClashMatchActivityAttributes.PrivateIntel? {
        guard normalizedStatus == "playing",
              cardsReadList.contains(where: { normalizedEmail($0) == normalizedEmail(viewer.email) }) else {
            return nil
        }

        if spectatorsList.contains(where: { normalizedEmail($0) == normalizedEmail(viewer.email) }) {
            return .init(ownerPlayerID: viewerID, role: .spectator)
        }
        if normalizedEmail(spyEmail ?? "") == normalizedEmail(viewer.email) {
            return .init(ownerPlayerID: viewerID, role: .spy)
        }
        return .init(
