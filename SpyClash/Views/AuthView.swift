...
final class AuthCinematicSoundPlayer {
    ...
    func preload() {
        // The system Apple/Google authorization UI can interrupt or deactivate
        // our audio session while these long-lived players remain cached. Always
        // reactivate before the early return so the next placement/upscale beat
        // cannot silently play into an inactive session.
        let engine = engine
        HapticManager.shared.prepareSharedAudioSession { isReady in
    ...
    func playFragmentLock(index: Int, totalCount: Int) {
        guard shouldAccentFragment(index: index, totalCount: totalCount) else { return }
        let engine = engine
        HapticManager.shared.prepareSharedAudioSession { isReady in
            guard isReady else { return }
            engine.playFragmentLock(index: index)
        }
    }
    ...
    func playCompletionSurge() {
        ...
        HapticManager.shared.prepareSharedAudioSession { isReady in
            guard isReady else { return }
            engine.playCompletionSurge()
        }
    }

    func stopFragmentLocks() {
        engine.stopFragmentLocks()
    }
    ...
    private func shouldAccentFragment(index: Int, totalCount: Int) -> Bool {
        guard totalCount > 0, index >= 0, index < totalCount else { return false }
        guard totalCount > 4 else { return true }

        let ordinal = index + 1
        let accents = Set([
            1,
            max(1, totalCount / 3),
            max(1, (totalCount * 2) / 3),
            totalCount
        ])
        return accents.contains(ordinal)
    }

    private final class Engine: @unchecked Sendable {
        private let queue = DispatchQueue(
            label: "com.spyclash.auth-cinematic-audio",
            qos: .userInitiated
        )

        // Every mutable field, including the AVAudioPlayer instances, is
        // isolated to `queue`. No audio decoding or player state touches main.
        private var fragmentPlayers: [AVAudioPlayer] = []
        private var completionPlayer: AVAudioPlayer?
        private var nextFragmentPlayer = 0
        private var lastFragmentPlayAt: [Int: ContinuousClock.Instant] = [:]
        private var lastCompletionPlayAt: ContinuousClock.Instant?
        ...
        func playFragmentLock(index: Int) {
            queue.async { [self] in
                preloadLocked()
                guard !fragmentPlayers.isEmpty else { return }

                let now = ContinuousClock.now
                if let previousPlay = lastFragmentPlayAt[index],
                   previousPlay.duration(to: now) < .milliseconds(140) {
                    return
                }
                lastFragmentPlayAt[index] = now

                let player = fragmentPlayers[nextFragmentPlayer % fragmentPlayers.count]
                nextFragmentPlayer += 1
                // The supplied lock sample contains about 58 ms of leading silence.
                // Seek past it so calls made at a placement deadline sound on that beat.
                if player.isPlaying { player.pause() }
                player.currentTime = min(0.058, player.duration)
                player.pan = 0
                player.rate = 1
                if !player.play() {
        ...
        func playCompletionSurge() {
            queue.async { [self] in
                ...
                let now = ContinuousClock.now
                if let previousPlay = lastCompletionPlayAt,
                   previousPlay.duration(to: now) < .milliseconds(750) {
                    return
                }
                lastCompletionPlayAt = now

                if completionPlayer.isPlaying { completionPlayer.pause() }
                completionPlayer.currentTime = 0
                if !completionPlayer.play() {
        ...
        func stopAll() {
            queue.async { [self] in
                stopFragmentLocksLocked()
                if completionPlayer?.isPlaying == true {
                    completionPlayer?.pause()
                    completionPlayer?.currentTime = 0
                }
                lastCompletionPlayAt = nil
            }
        }

        private func preloadLocked() {
            guard fragmentPlayers.isEmpty || completionPlayer == nil else { return }

            if fragmentPlayers.isEmpty, let fragmentURL = Bundle.main.url(
                forResource: "apple-fragment-lock",
                withExtension: "wav"
            ) {
                fragmentPlayers = (0..<2).compactMap { _ in
                    makePlayer(url: fragmentURL, volume: 0.30)
                }
            }

            if completionPlayer == nil, let completionURL = Bundle.main.url(
                forResource: "apple-access-surge",
                withExtension: "wav"
            ) {
                completionPlayer = makePlayer(url: completionURL, volume: 0.58)
            }
        }

        private func stopFragmentLocksLocked() {
            for player in fragmentPlayers where player.isPlaying {
                player.pause()
