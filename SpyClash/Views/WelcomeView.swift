        ...
        case .privacy:
            [
                ...
                LegalSection(
                    ...
                    text: "No vendemos tu información personal. Base44 proporciona autenticación, almacenamiento de datos de la aplicación, funciones de servidor, registros con acceso controlado y métricas operativas limitadas a los campos descritos anteriormente. Para generar paquetes de palabras mediante IA, primero se prueba el endpoint directo de IA configurado, que de forma predeterminada es la Responses API de OpenAI, con la opción store de la solicitud establecida en false. Ante determinados fallos operativos o de configuración, la misma entrada también puede procesarse mediante Base44 InvokeLLM; Base44 puede procesar esa solicitud a través de su proveedor de modelos de IA configurado. El procesamiento y la conservación por parte del proveedor se rigen por sus términos y configuración aplicables. Apple procesa datos para Sign in with Apple, la conciliación de transacciones heredadas, las notificaciones APNs y la entrega mediante ActivityKit. Para entregar notificaciones y Live Activities, el backend de Base44 envía a Apple el token y la alerta correspondiente o la carga útil del estado público de la partida. Estas cargas útiles pueden incluir nombres para mostrar, símbolos de avatar, estado de participantes, ronda, categoría pública, temporizador e identificadores de navegación, pero no correo electrónico, código para entrar en la sala, rol ni palabra secreta. Google procesa el inicio de sesión con Google y Stripe procesa los registros conservados de facturación web heredada. Limitamos las divulgaciones a las funciones del servicio descritas anteriormente y configuramos y supervisamos a nuestros proveedores de servicios para proteger los datos personales de conformidad con esta política y la legislación aplicable. Tu nombre para mostrar, avatar, comentarios de perfil, estadísticas competitivas, amigos aceptados y el contenido que decidas compartir pueden ser visibles para otros jugadores de SpyClash. Un paquete de palabras personalizado puede mostrarse a los participantes cuando lo seleccionas para una partida. Los reportes de Comunidad y sus instantáneas no son públicos y solo están disponibles para administradores autorizados y proveedores de servicios necesarios."
                ),
                LegalSection(
        ...
        case .privacy:
            [
                ...
                LegalSection(
                    ...
                    text: "Мы не продаем вашу личную информацию. Base44 обеспечивает аутентификацию, хранение данных приложения, серверные функции, записи с контролируемым доступом и операционные метрики, ограниченные описанными выше полями. При создании наборов слов с помощью ИИ сначала используется настроенная прямая конечная точка ИИ, которой по умолчанию является Responses API OpenAI; параметр store запроса при этом установлен в false. При определенных операционных ошибках или ошибках конфигурации те же входные данные также могут обрабатываться через Base44 InvokeLLM; Base44 может обработать такой запрос с помощью настроенного поставщика модели ИИ. Обработка и хранение данных поставщиком регулируются применимыми условиями и конфигурацией соответствующего поставщика. Apple обрабатывает данные для Sign in with Apple, сверки ранее совершенных транзакций, уведомлений APNs и доставки ActivityKit. Для доставки уведомлений и Live Activities серверная часть Base44 передает Apple токен и соответствующее уведомление или полезную нагрузку публичного состояния матча. Такая полезная нагрузка может включать отображаемые имена, символы аватаров, статус участников, раунд, публичную категорию, таймер и идентификаторы навигации, но не адрес электронной почты, код входа в комнату, роль или секретное слово. Google обрабатывает вход через Google, а Stripe — сохраненные записи ранее действовавшего веб-биллинга. Мы ограничиваем передачу данных описанными выше функциями сервиса, а также настраиваем и контролируем поставщиков услуг, чтобы личные данные защищались в соответствии с этой политикой и применимым законодательством. Ваше отображаемое имя, аватар, комментарии в профиле, соревновательная статистика, принятые друзья и содержимое, которым вы решили поделиться, могут быть видны другим игрокам SpyClash. Пользовательский набор слов может быть показан участникам, когда вы выбираете его для игры. Жалобы Сообщества и их снимки не являются публичными и доступны только уполномоченным администраторам и необходимым поставщикам услуг."
                ),
                LegalSection(
        ...
        case .privacy:
            [
                ...
                LegalSection(
                    ...
                    text: "Ми не продаємо вашу особисту інформацію. Base44 забезпечує автентифікацію, зберігання даних застосунку, серверні функції, записи з контрольованим доступом та операційні метрики, обмежені описаними вище полями. Під час створення наборів слів за допомогою ШІ спочатку використовується налаштована пряма кінцева точка ШІ, якою за замовчуванням є Responses API OpenAI; параметр store запиту встановлено в false. У разі певних операційних помилок або помилок конфігурації ті самі вхідні дані також можуть оброблятися через Base44 InvokeLLM; Base44 може обробити такий запит за допомогою налаштованого постачальника моделі ШІ. Обробка та зберігання даних постачальником регулюються застосовними умовами й конфігурацією відповідного постачальника. Apple обробляє дані для Sign in with Apple, звіряння раніше здійснених транзакцій, сповіщень APNs і доставлення ActivityKit. Для доставлення сповіщень і Live Activities серверна частина Base44 передає Apple токен і відповідне сповіщення або корисне навантаження публічного стану матчу. Таке корисне навантаження може містити імена для відображення, символи аватарів, статус учасників, раунд, публічну категорію, таймер та ідентифікатори навігації, але не адресу електронної пошти, код входу до кімнати, роль або секретне слово. Google обробляє вхід через Google, а Stripe — збережені записи раніше чинного веббілінгу. Ми обмежуємо передавання даних описаними вище функціями сервісу, а також налаштовуємо й контролюємо постачальників послуг, щоб особисті дані захищалися відповідно до цієї політики та застосовного законодавства. Ваше ім’я для відображення, аватар, коментарі у профілі, змагальна статистика, прийняті друзі та вміст, яким ви вирішили поділитися, можуть бути видимими іншим гравцям SpyClash. Власний набір слів може бути показаний учасникам, коли ви вибираєте його для гри. Скарги Спільноти та їхні знімки не є публічними й доступні лише уповноваженим адміністраторам і необхідним постачальникам послуг."
                ),
                LegalSection(
