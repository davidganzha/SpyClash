        ...
        case .privacy:
            [
                LegalSection(
                    ...
                    text: "We collect information you provide directly, including your email address, display name, avatar, profile comments, and custom word packs. When you submit a Community report, we store the selected reason, the reporter and reported account identifiers, and a private snapshot of the reported comment when applicable. We store account identifiers, private room and game state, match history, scores, gameplay statistics, and AI-generation requests and cached results needed to operate the service. To deliver notifications and Live Activities, we collect a randomly generated installation identifier, APNs and ActivityKit push tokens, notification authorization status and preferences, app version, and selected language or locale. The server stores a one-way hash of the installation identifier and encrypts raw push tokens at rest. These identifiers are linked to your signed-in account only to deliver requested game, friend, room, and match updates and are not used for advertising or tracking. For accounts with a legacy provider agreement, we may retain transaction or subscription identifiers, lifecycle status, and dates; we do not receive full payment-card details. QR camera frames and ARKit Camera Assistance data used to stabilize local Nearby Interaction/Radar ranging are processed on device and are not uploaded or retained."
                ),
                LegalSection(
